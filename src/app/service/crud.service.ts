...
export class CrudService {
  ...
  constructor() {
    ...
    if(storage == ""){
      localStorage.setItem("books", JSON.stringify({favorite: [], saved: [], finished: []}));
    }
  }
  ...
  deleteBook(id: number, filter: string){
    ...
    if(filter == 'favorite'){
      const updated_storage = storage.favorite.filter((bookStorage: IResponseAPI) => bookStorage.id !== id);
    ...
    if(filter == 'finished'){
      const updated_storage = storage.finished.filter((bookStorage: IResponseAPI) => bookStorage.id !== id);
    ...
    if(filter == 'saved'){
      const updated_storage = storage.saved.filter((bookStorage: IResponseAPI) => bookStorage.id !== id);
